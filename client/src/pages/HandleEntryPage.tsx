...
type HandleForm = z.infer<typeof handleSchema>;
 ...
 const HandleEntryPage = () => {
  ...
  if (onboardingCompleted) return <Navigate to="/onboarding/ingesting" replace />;
  ...
  const onSubmit = async ({ handle }: HandleForm) => {
    try {
      ...
      if (user) useAuthStore.getState().setUser({ ...user, onboardingCompleted: true });
      useIngestStore.getState().setIngestActive(true);
    } catch {
      setError('root', { message: 'Could not link that handle. Check it and try again.' });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mx-auto flex max-w-sm flex-col gap-4">
      ...
      <input {...register('handle')} placeholder="e.g. tourist" className="rounded border px-3 py-2" />
      {errors.handle && <p className="text-sm text-red-600">{errors.handle.message}</p>}
      {errors.root && <p className="text-sm text-red-600">{errors.root.message}</p>}
      <button type="submit" disabled={isSubmitting} className="rounded bg-slate-900 py-2 text-white disabled:opacity-50">

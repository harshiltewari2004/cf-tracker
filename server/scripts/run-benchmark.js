...
const run = async () => {
  ...
  if (fs.existsSync(CACHE_PATH)) {
    ...
    logger.info(
      { N: injectedSelection.cohort.N },
      "loaded cohort from cache — skipping the live scan",
    );
  }
  ...
  const result = await BenchmarkEngine.refresh({
    ...
    onSelection: (selected) => {
      ...
      logger.info(
        { N: selected.cohort.N },
        "cohort cached to disk — re-runs skip the scan",
      );
    },
  });
  ...
  if (!result) {
    logger.warn(
      "benchmark held previous version (cohort below floor) — nothing written",
    );
    return;
  }

  const rows = await BenchmarkTargetCount.find({
    cohortVersion: result.version,
  })
    .sort({ p50: -1 })
    .limit(15)
    .lean();
  ...
  logger.info(
    ...
    "benchmark written — top 15 buckets by p50:",
  );
  console.table(
    rows.map((r) => ({ topic: r.topic, bucket: r.bucket, p50: r.p50 })),
  );
};

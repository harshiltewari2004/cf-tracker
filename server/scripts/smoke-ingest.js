...
if (!handle) {
  console.error('Usage: node scripts/smoke-ingest.js <cfHandle> [--wipe] [--refresh]');
  process.exit(1);
}
...
const wipeSmokeData = async (userId) => {
  ...
  await CFProfile.updateOne(
    { user: userId },
    { $set: { ingestStatus: 'pending' }, $unset: { lastIngestedSubmissionId: 1 } }
  );
...
const report = async (userId, job, elapsedSec) => {
  ...
  const crIds = new Set(
    (await ContestResult.find({ user: userId }).select('_id').lean()).map((c) => String(c._id))
  );
  const cprs = await ContestProblemResult.find({ user: userId }).select('contestResult').lean();
  const orphans = cprs.filter((c) => !crIds.has(String(c.contestResult))).length;
  ...
  console.log(`job.lastIngestedSubmissionId:  ${freshJob.lastIngestedSubmissionId} (resume cursor: SMALLEST seen)`);
  console.log(`profile.ingestStatus:          ${profile.ingestStatus}`);
  console.log(`profile.lastIngestedSubmissionId: ${profile.lastIngestedSubmissionId} (committed: NEWEST seen)`);
  console.log('----------------------------------------------');
  ...
  console.log('----------------------------------------------');
  console.log(`distinct CONTESTANT contests:  ${contestantContestIds.filter(Boolean).length}`);
  console.log(`ContestResult count:           ${crCount}  (gap vs distinct = skipped: no rating change / not in catalog — check warn logs)`);
...
const main = async () => {
  ...
  if (!profile) {
    profile = await CFProfile.create({ user: user._id, handle, ingestStatus: 'pending' });
  } else if (profile.handle !== handle) {
    console.error(`Existing smoke profile has handle '${profile.handle}'. Use --wipe to switch.`);
    process.exit(1);
  }
  ...
  console.log(`-- running ${refresh ? 'runDailyRefresh' : 'runInitialIngest'} for '${handle}' --`);
  const t0 = Date.now();
  if (refresh) {

...
import { extractContestFails, seedUpsolveQueue } from '../engines/ContestFeedbackEngine.js';
...
const run = async () => {
  ...
  const gated = await seedUpsolveQueue(
    testUserId, signupDate, 1899, beforeSignup, [failedProblem]
  );
  console.log('\n--- gate (pre-signup) ---');
  console.log('seeded (expect 0):', gated);

  // First real seed: should insert one pending row.
  await seedUpsolveQueue(testUserId, signupDate, 1900, afterSignup, [failedProblem]);
  let row = await UpsolveQueue.findOne({ user: testUserId, problem: failedProblem._id }).lean();
  console.log('\n--- first seed ---');
  console.log('status (expect pending):', row.status);
  console.log('scheduledFor - addedAt (expect 86400000 ms):',
  ...
  await seedUpsolveQueue(testUserId, signupDate, 1950, afterSignup, [failedProblem]);
  row = await UpsolveQueue.findOne({ user: testUserId, problem: failedProblem._id }).lean();

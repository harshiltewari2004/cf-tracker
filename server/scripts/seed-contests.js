...
const main = async () => {
  ...
  for (const p of problems) {
    if (!problemsByContest.has(p.cfContestId))
      problemsByContest.set(p.cfContestId, []);
    problemsByContest
      .get(p.cfContestId)
      .push({ problemIndex: p.cfIndex, problem: p._id });
  }
  ...
  for (const contest of contests) {
    ...
    if (contestProblems.length === 0) {
      ...
      logger.warn(
        { cfContestId: contest.id, name: contest.name },
        "seed: Div2 contest has no problems in catalog",
      );
  ...
  logger.info(
    { counts, stored, storedWithNoProblems },
    "seed: contest catalog complete",
  );
  console.log("\n=== seed summary ===");

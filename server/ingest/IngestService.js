...
const ingestSubmissionPages = async({userId,handle,job,floorSubmissionId})=>{
    let from = 1;
    ...
    for(;;){
        const page = await cfApiClient.getUserStatus(handle,from,INGEST_PAGE_SIZE);
        if(page.length==0){
            break;
        }
        ...
        for(const raw of page){
            ...
            if(resumeCursor!=null&&parsed.cfSubmissionId>=resumeCursor)continue;
        ...
        if(docs.length>0){
            try{
                ...
                if(err.code===MONGO_DUPLICATE_KEY_CODE){
                    ...
                    logger.warn({userId,page:from},'duplicate submissions absorbed on re-ingest');
                }
                else{
                    throw err;
                }
...
const deriveContestResults = async({userId,handle,contestIds})=>{
    ...
    const ratingByContest = new Map(ratingChanges.map((rc)=>[rc.contestId,rc]));

    for(const cfContestId of contestIds){
        const contest = await Contest.findOne({cfContestId}).lean();

        if(!contest){
            logger.warn({userId,cfContestId},'contest not in catalog,skipping result derivation');
            continue;
        }

        const rc = ratingByContest.get(cfContestId);
        if(!rc){
            logger.warn({userId,cfContestId},'no rating change for contest,skipping');
            continue;
        }
        ...
        const attempts = (byIndex.get(problemIndex)??[]).sort(
            ...
            ?attempts.filter((a)=>a.verdict!=='OK'&&a.submittedAt<firstAC.submittedAt).length
            ...
            return{
                problemIndex,
                problem,
                status:firstAC?'solved':attempts.length>0?'failed':'unattempted',
                firstACTime:firstAC?differenceInMinutes(firstAC.submittedAt,contest.startTime):null,
                failCount,
...
export const runDailyRefresh = async({userId,ingestJobId})=>{
    ...
    if(floorSubmissionId==null){
        logger.warn({userId},'daily refresh with no committed cursor;running full history');
    }
    job.status = 'processing';
    job.startedAt = job.startedAt??new Date();
    await job.save();
  ...
  missedProblems: summary.missedProblems,
    ...
    logger.info(
        { userId,ingestJobId,submissionsIngested:summary.submissionsIngested},'daily refersh compelte');
};